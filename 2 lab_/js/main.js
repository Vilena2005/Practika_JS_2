Vue.component('note-app',{
    template:`
        <div>
            <header>
                <h1 class="app-title">NoteBook</h1>
                <task-create @create-task="addTask" :taskCount="tasks.length"></task-create>
                <button @click="clearAllTasks">Очистить все заметки</button>
            </header>
            <main>
                <div class="tasks">
...
Vue.component('task-create', {
    template:`
        <div class="create-task">
            ...
            <button @click="createTask" v-if="title !== '' && itemOne !== '' && itemTwo !== '' && itemThree !== '' && itemFour !== ''" :disabled="isTaskLimitReached ">Create task</button>
            ...
            >
                ...
                <ul class="task-list">
                    <li class="task-list-item" v-for="item in task.items" :key="item.id">
                        ...
                        @change="checkItems(task, completedTasks, item)" :disabled="isTaskProcessFull || ifChecked(item)"> 
                        <p>{{ item.text }}</p>
                    </li>
                </ul>
            </div>
